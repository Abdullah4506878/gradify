...
function taskMatchesSearch(task: Task, query: string): boolean {
  ...
  const fypId = (task.group.fypId ?? '').toLowerCase();
  if (fypId.includes(q)) return true;
  ...
  for (const status of task.memberStatuses) {
    const name = (status.user.name ?? '').toLowerCase();
...
export default function ManagerTasksPage() {
  ...
  const filtered = tasks.filter((t) => {
    const matchesSearch = taskMatchesSearch(t, search);
